...
public class LongestCommonSubsequenceProblem {
    ...
    /**
     * Cria uma instância do resolvedor do problema da subsequência comum
     * máxima e resolve o problema para os dados passados.
     *
     * @param string1 Uma string que será comparada
     * @param string2 A outra string que será comparada
     */
    public LongestCommonSubsequenceProblem( String string1, String string2 ) {
    ...
    private void solve( String string1, String string2 ) {
         ...
         * c[i][j] = max( c[i-1][j], c[i][j-1] ),                   se i e j > 0 e string1[i] != string2[j]
         */
    ...
    private void processPaths() {
        ...
        // uma das duas strings é vazia: não há subsequência comum
        if ( i == 0 || j == 0 ) {
            return;
        }
